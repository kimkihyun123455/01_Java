...
public class FileService {
	...
	public void method2() {
		...
		if(!file.exists()) {
			...
			try {
				//File 클래스의 메서드는 대부분 IOException을 발생시킨다
				//IOException은 반드시 예외처리를 해야한다
				
				if(file.createNewFile()) {//파일이 정상 생성되었을 경우
					
					System.out.println(file.getName()+"파일이 정상 생성되었습니다");
					
				}	
				
			}catch(IOException e) {
				e.printStackTrace();//예외 정보 + 발생 위치 추적 메서드
			}
		}
	...
	public void method3() {
		...
		for(File f : files) {
			...
			//java.text.SimpleDateFormat : 간단히 날짜 형식을 지정
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd a h:mm");
														//2025-03-05 오전 10:33
			
			//String SimpleDateFormat.format(long) : 
			//매개변수 long 값을 지정된 패턴 형식으로 변환하여 문자열 형식으로 반환
			String date = sdf.format(lastMod);
			...
			if(f.isDirectory()) size = ""; //폴더라면 사이즈 빈문자열//폴더의 크기 메서드는 존재x
